	...
	if !ok {
		if name == "" {
			...
			publishedStorage, err = s3.NewPublishedStorage(
				...
				params.EncryptionMethod, params.uploadPartSize, params.PlusWorkaround, params.DisableMultiDel,
				params.ForceSigV2, params.ForceVirtualHostedStyle, params.MultipartUpload, params.Debug)
			if err != nil {
				Fatal(err)
			}
